<template>
	<div class="info">
		<van-nav-bar title="个人资料" left-text="返回" right-text="保存" left-arrow @click-left="back" @click-right="alter" />
		...
		<div class="line clearfix">
			...
			<div class="right">
				<van-field readonly clickable input-align="right" name="sex" :value="sex" placeholder="点击选择性别" :default-index="sexIndex" @click="showSex = true" />
				<van-popup v-model="showSex" position="bottom">
					<van-picker show-toolbar :columns="sexList" @confirm="chooseSex" @cancel="showSex = false" />
				</van-popup>
			</div>
		</div>
		<div class="line clearfix">
			...
			<div class="right">
				<van-field readonly clickable input-align="right" name="birth" :value="birth" placeholder="点击选择时间" @click="showBirth = true" />
				<van-popup v-model="showBirth" position="bottom">
					<van-datetime-picker type="date" v-model="currentDate" @confirm="chooseBirth" @cancel="showBirth = false" :min-date="time.start" :max-date="time.end" />
				</van-popup>
			</div>
		</div>
		<div>
